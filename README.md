...
The authenticated `GET /api/cron/screener` endpoint is the write boundary: it discovers the complete candidate universe, fetches daily OHLCV, calculates the deterministic swing rating, and writes the snapshot only after the scan completes. A failed run does not overwrite the prior stored run. The dashboard accepts the latest result for the current completed weekday session; before the 16:30 WIB cutoff and during weekends it retains the prior weekday snapshot. An unavailable or failed current-day run is shown as unavailable rather than silently triggering a partial browser scan.
...
- Apply `supabase/migrations/202609060001_swing_screener.sql` to your Supabase database before enabling the dashboard. It creates `public.swing_screening_runs`, an RLS-protected table containing atomic, versioned scan snapshots; old BSJP runs remain separate. Hermes is responsible for calling the authenticated scan endpoint after market close. Vercel no longer schedules the screener job.
...
Run the production build on Node 22, invoke authenticated `/api/cron/screener`, then verify `/api/screener` from the actual deployment host. Yahoo access is unofficial and can be blocked or rate-limited by server IP; local success does not establish availability on Vercel. The app reports missing daily data instead of falling back to fabricated or partial results. No Yahoo cookies, credentials or API keys need to be configured. Existing Supabase schema works unchanged because provider identity is part of the snapshot cache key.
