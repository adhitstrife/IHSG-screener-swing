...
The authenticated `/api/cron/screener` endpoint is the write boundary. Hermes calls its batch API sequentially: POST `{ "offset": 0 }` starts a run and returns a `universeId` plus `nextOffset`; POST `{ "offset": nextOffset, "universeId": "..." }` continues it. Each request processes three symbols, stores its server-created batch, and returns quickly. The final batch atomically publishes the complete snapshot. A failed batch does not overwrite the prior stored run. The dashboard accepts the latest result for the current completed weekday session; before the 16:30 WIB cutoff and during weekends it retains the prior weekday snapshot. An unavailable or failed current-day run is shown as unavailable rather than silently triggering a partial browser scan.
