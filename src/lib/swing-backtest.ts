...
export function calculateSwingTrades(symbol: string, candles: MarketCandle[], minimumScore = SWING_RULES.minimumScore as number, benchmarkCandles?: MarketCandle[], evaluate = evaluateSwing, maxHoldingSessions = SWING_RULES.maxHoldingSessions as number) {
  const trades: SwingTrade[] = [];
  // Reserve the complete holding window. No fabricated end-of-data liquidations.
  for (let i = SWING_RULES.minimumHistory - 1; i < candles.length - maxHoldingSessions; i++) {
    const signal = evaluate(candles.slice(0, i + 1), { benchmarkCandles: benchmarkCandles?.filter((bar) => bar.date <= candles[i].date) });
