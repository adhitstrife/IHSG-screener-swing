...
export async function getStockAnalysisContext(symbol: string): Promise<StockAnalysisContext> {
  ...
  const history = await getDailyHistory(symbol, format(from), format(to));
...
export type StockAiAnalysis = { source: string; symbol: string; asOf: string; verdict: AiVerdict; confidence: number; summary: string; positives: string[]; risks: string[]; levels: { buyTarget: number | null; sellTarget: number | null; cutLoss: number | null; rationale: string }; tradePlan: { entryCondition: string; invalidation: string; exitPlan: string }; newsCoverage: "not-connected"; disclaimer: string };
