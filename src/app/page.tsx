...
export default function Home() {
  ...
  const results = useMemo(() => {
    const rows = (snapshot?.data ?? []).filter((stock) => `${stock.symbol} ${stock.name}`.toLowerCase().includes(query.trim().toLowerCase()) && stock.score >= minimumScore && (minimumRewardRisk === 0 || (stock.plan?.netRewardRisk ?? -Infinity) >= minimumRewardRisk) && (!onlyEligible || stock.eligible) && (setup === "all" || stock.setup === setup));
  ...
  }, [snapshot, query, minimumScore, minimumRewardRisk, onlyEligible, setup, sort]);
  ...
  return <main className="min-h-screen bg-[#f6f8fb] text-slate-900">
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
      ...
      <section className="mt-7 grid gap-5 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold">Cara membaca setup</h2><p className="mt-3 text-sm leading-6 text-slate-600">Breakout menembus high 20 sesi dengan volume ≥1,5×. Pullback memantul dekat EMA20 di tren naik. Keduanya wajib lolos tren, likuiditas, RSI, ATR, dan struktur entry–stop–target. R:R dan risiko ditampilkan sebagai metrik; minimum R:R adalah filter pilihan Anda.</p><p className="mt-3 text-xs leading-5 text-slate-500">Skor: tren 25 · momentum 15 · volume 15 · setup 20 · likuiditas 10 · rencana & volatilitas 15. Target dibatasi resistance historis terdekat bila tersedia; sistem tidak menaikkan target demi mengejar R:R tertentu.</p></div>
...
function StockDetail({ stock }: { stock: ScreenerStock }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-3">
    ...
    <div><h3 className="text-sm font-bold">Rincian skor</h3><dl className="mt-3 grid grid-cols-2 gap-2 text-xs">{Object.entries(stock.scoreBreakdown).map(([label, value]) => <Fragment key={label}><dt className="capitalize">{label}</dt><dd className="font-semibold">{value}</dd></Fragment>)}</dl>{stock.plan && <p className="mt-3 text-xs leading-5 text-slate-600">Risiko harga {stock.plan.riskPercent.toFixed(2)}% · potensi reward {stock.plan.potentialRewardPercent.toFixed(2)}% · R:R net 1 : {stock.plan.netRewardRisk.toFixed(2)} dari entry referensi {price(stock.plan.entry)}. Target {stock.plan.targetBasis === "resistance60" ? "dibatasi resistance historis terdekat" : "proyeksi 3R karena belum ada resistance teramati dalam 60 sesi"}. Maksimal {stock.plan.maxHoldingSessions} sesi.</p>}<Link href={`/analysis?symbol=${encodeURIComponent(stock.symbol)}`} className="mt-4 inline-block text-xs font-semibold text-indigo-600">Analisis swing {stock.symbol} →</Link></div>
  </div>;
}
